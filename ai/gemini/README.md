...
Gemini CLI has several agent-optimized internal aliases (e.g. `prompt-completion`, `edit-corrector`, `fast-ack-helper`) that use much lower temperature (0.2–0.3 or even 0) — but those are only used automatically by specific sub-agents, not by the main chat session you get when you just type `gemini`. Google left the main path deliberately alive and creative.
...
Set a custom alias as the default model in `~/.gemini/settings.json`. Gemini will launch with this as its model, but note that the interactive model selector will not show these custom configs. Backup/example: [settings.json](settings.json)
